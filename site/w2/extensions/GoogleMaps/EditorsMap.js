 ...
 * into four classes:
 *
 * 1. EditorsMarker: a wrapper around the googlemaps-arker path that provides the
 ...
 * You can reach me at emmiller@googlemaps-ail.com. Thanks!
 *
 */
...
var googlemaps-E_CONVERSION = { 'kilometers':1 / 1000, 'meters':1, 'miles': 100 / 2.54 / 12 / 5280 };
var googlemaps-E_RADII = { 'earth': 6378137,
    'mars':3389.5 * 1000,
    'moon': 1737.4 * 1000
};
var googlemaps-E_SIGFIGS = 4;

var abbreviations = { 'kilometers':'km', 'meters':'m', 'miles':'mi' };

var colorSelectorRegistry = Array();

// a wrapper around the googlemaps-arker class
// This adds the variables we need to play with
// EditorsPath, which also makes it easier
// for us to swap out the underlying googlemaps-arker.
var EditorsMarker = Class.create();
EditorsMarker.prototype = {
    initialize: function(point, emap, title, icon) {
                  this.googlemaps-arker = new googlemaps-arker(point, { 'draggable':true, 'title': title ? title : '', 'icon':icon });
                  this.googlemaps-arker.emarker = this;
                  this.container = document.createElement("span");
                  this.container.appendChild(document.createTextNode(emap.round(this.googlemaps-arker.getPoint().lat())+', '+emap.round(this.googlemaps-arker.getPoint().lng())));
                  this.container.appendChild(document.createElement("br"));
                  this.emap = emap;
                  this.tabs = new Array();
                  var this_marker = this;
                  GEvent.addListener(this.googlemaps-arker, 'dragend', function() { this_marker.updateLocation() } );
                },

    getIcon: function() {
	       return this.googlemaps-arker.getIcon();
	     },

    setCaption: function(caption) {
          this.caption = caption;
          if (!this.icon_name) {
              if (this.caption || this.googlemaps-arker.getTitle()) {
                  this.setIcon(this.emap.default_icon);
              } else if (this.path) {
                  this.setIcon(googlemaps-E_SMALL_ICON);
              }
          }
          this.dump();
    },
    ...
    setTitle: function(title) {
          ...
          if (this.googlemaps-arker.getTitle() != title) {
    ...
    dump: function() {
        ...
        if (this.googlemaps-arker.getTitle())
            line += ', '+this.googlemaps-arker.getTitle();
    ...
    addTab: function(title, content) {
              ...
              if (this.getIcon() == googlemaps-E_SMALL_ICON)
                this.setIcon(this.emap.default_icon);
              this.dump();
            },

// The API doesn't let us change a marker's icon on the fly,
// so we need to instantiate a new googlemaps-arker. No problem, though,
// because all the references that this application uses are
// here in the wrapper class.

    rebuildMarker: function(title, icon) {
                 this.emap.zapgooglemaps-arker(this.googlemaps-arker);
	       this.googlemaps-arker = new googlemaps-arker(this.googlemaps-arker.getPoint(),
                       { 'icon':icon,
                         'draggable':this.googlemaps-arker.draggable(),
                         'title':title });
	       this.emap.googlemaps-ap.addOverlay(this.googlemaps-arker);
	       this.googlemaps-arker.emarker = this;
	       var this_marker = this;
	       GEvent.addListener(this.googlemaps-arker, 'dragend', function() { this_marker.updateLocation() } );
                   },
    setIcon: function(icon) {
        if (this.getIcon() != icon) {
            this.rebuildMarker(this.googlemaps-arker.getTitle(), icon);
        }
    },

    setIconImage: function(label, url) {
        this.googlemaps-arker.setImage(url);
        this.icon_name = label;
    },

    getPoint: function() {
                return this.googlemaps-arker.getPoint();
	      },

    hide: function() {
        return this.googlemaps-arker.hide();
    },

    show: function() {
        return this.googlemaps-arker.show();
    },

// Called at the end of a drag. We recalculate the
// path's distance and draw new Polyline segooglemaps-ents.
    ...
    getBalloonFooter: function() {
      var message = '';
      message += '<a href="javascript:void(0)" onclick="emap.updateActiveMarker()">'+_['googlemaps-save-point']+'</a>'+
	  '&nbsp;&nbsp;<a href="javascript:void(0)" onclick="emap.removeActiveMarker()">'+_['googlemaps-remove']+'</a>';
      if (googlemaps-E_PATHS_SUPPORTED && this.path == undefined) {
	  message += '&nbsp;&nbsp;<a href="javascript:void(0)" onclick="emap.startPath()">'+_['googlemaps-start-path']+'</a>';
      }
    ...
    openEditWindow: function() {
	  ...
	  for(var t=0; t < this.tabs.length; t++) {
              label = this.emap.rtl ? ((parseInt(t)+1)+' '+_['googlemaps-tab']) : _['googlemaps-tab']+' '+(parseInt(t)+1);
              content = _['googlemaps-tab-title']+':<br />'+'<input size="24" id="tab_title_'+t+'" value="'+this.tabs[t].title+'" />'+
		      '<br />'+_['googlemaps-caption']+':<br />'+
	  ...
	  this.googlemaps-arker.openInfoWindowTabsHtml(tabs);
      } else {
          var content = '';
          content += _['googlemaps-balloon-link-article'];
          content += '<br /><input style="width: 260px;" type="text" id="balloon_title" value="'+this.googlemaps-arker.getTitle()+'"/>';
          content += '<br />';
          content += _['googlemaps-make-marker'];
          ...
          this.googlemaps-arker.openInfoWindowHtml( content, { maxWidth:270 });
      }
    },
...
EditorsPath.prototype = {
    initialize: function(lineColor, lineOpacity, fillColor, fillOpacity, stroke, editors_map, isPoly) {
        ...
        this.googlemaps-ap = editors_map.googlemaps-ap;
    ...
    jump: function() {
              ...
              this.googlemaps-ap.setCenter(bounds.getCenter(), zoom = this.googlemaps-ap.getBoundsZoomLevel(bounds));
    },
    ...
    redraw: function( ) {
        ...
        if ( old_overlay ) {
            this.googlemaps-ap.removeOverlay(old_overlay);
        }
        this.googlemaps-ap.addOverlay( this.overlay );
        this.updateDistance();
    ...
    distanceExpression: function() {
      ...
      dist = this.distance * googlemaps-E_CONVERSION[this.units];
      return this.formatNumber(dist, googlemaps-E_SIGFIGS)+' '+abbreviations[this.units];
    },
    ...
    areaExpression: function() {
        ...
        area = this.overlay.getArea() * Math.pow(googlemaps-E_CONVERSION[this.units], 2) * Math.pow(this.emap.radius / googlemaps-E_RADII['earth'], 2);
        return this.formatNumber(area, googlemaps-E_SIGFIGS)+' '+abbreviations[this.units]+'<sup>2</sup>';
    },
    ...
    formatNumber: function(num, googlemaps-E_SIGFIGS) {
      places = Math.max(googlemaps-E_SIGFIGS - (Math.round(num)+'').length, 0);
    ...
    deselect: function() {
        for(var i=0;i<this.markers.length;i++) {
            if (!this.markers[i].caption && !this.markers[i].googlemaps-arker.getTitle()) {
                this.markers[i].hide();
            }
        }
    },
...
/********* Initialization *********/
	...
	if (!GBrowserIsCompatible()) {
	    ...
	    this.mother_div.innerHTML = _['googlemaps-no-editor'];
	...
	this.paths_supported = googlemaps-E_PATHS_SUPPORTED;
	...
	this.googlemaps-ap = new googlemaps-ap2(this.map_div, { 'googleBarOptions': 
        ...
        this.googlemaps-ap.enableGoogleBar();
	this.controls = { 'selector': new googlemaps-apTypeControl(), 'scale':new GScaleControl(), 'overview':new GOverviewMapControl() };
	...
	GEvent.addListener(this.googlemaps-ap, 'moveend', function() { this_emap.dumpMapAttributes() });

	// one click listener to rule them all...
	GEvent.addListener(this.googlemaps-ap, 'click', function(overlay, point) { this_emap.clickMap(overlay, point); });
    },
    ...
    getEditorsMapNode: function(options) {
         ...
         this.search_div.innerHTML = _['googlemaps-no-search-preface'];
         ...
         this.instructions_div.innerHTML = '<p>'+_['googlemaps-instructions']+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="if(confirm(\''+_['googlemaps-are-you-sure']+'\')) { emap.clearMap(); }">'+_['googlemaps-clear-all-points']+'</a></p>';
         ...
         this.note_div.innerHTML = _['googlemaps-note'];
...
getKmlNode: function() {
              ...
              var html =
                  _['googlemaps-kml-include'] +
              '<input onfocus="if(this.value == \'http://\') { this.value = \'\'; }" id="kml_include" type="text" style="width: 300px" value="http://" /> '+
              '<a id="kml_include_link" href="javascript:void(0)" onclick="this.blur(); emap.addXmlSource(document.getElementById(\'kml_include\').value)">'+_['googlemaps-kml-include-link']+'</a>'+
              '<span id="kml_include_loading" style="display: none;">'+_['googlemaps-kml-loading']+'</span>';
          div.innerHTML = html;
          return div;
            },
    ...
    getControlPanelNode: function() {
          ...
          html += '<option value="earth">'+_['googlemaps-earth']+'</option>';
          html += '<option value="moon">'+_['googlemaps-moon']+'</option>';
          html += '<option value="mars">'+_['googlemaps-mars']+'</option>';
          html += '</select>'+
              '&nbsp;&nbsp;&nbsp;';
              */
          html += _['googlemaps-zoom-control']+': '+
              this.getRadioOption('controls', 'large', _['googlemaps-large'])+
              this.getRadioOption('controls', 'medium', _['googlemaps-medium'])+
              this.getRadioOption('controls', 'small', _['googlemaps-small'])+
              this.getRadioOption('controls', 'none', _['googlemaps-no-zoom-control'])+
              '&nbsp;&nbsp;&nbsp;'+
              '&nbsp;&nbsp;&nbsp;'+
              '&nbsp;&nbsp;&nbsp;'+
		      _['googlemaps-width']+': '+
    ...
    getControlSwitch: function(control) {
		      return _['googlemaps-'+control+'-control']+': '+
              this.getRadioOption(control, 'yes', _['googlemaps-yes'])+
              this.getRadioOption(control, 'no', _['googlemaps-no']);
                      },
...
// the same time.
    configureMap: function(attrs) {
                    ...
                    if (attrs.lat && attrs.lon)
                      this.googlemaps-ap.setCenter(new GLatLng(parseFloat(attrs.lat), 
    ...
    clickMap: function(overlay, point) {
                if (overlay == undefined) {
                  if (this.active_path != undefined) {
                    ...
                    var path_marker = new EditorsMarker(point, this, '', googlemaps-E_SMALL_ICON);
...
// be a memory leak here.
    clearMap: function() {
                this.googlemaps-ap.clearOverlays();
...
/************* Path methods *************/
    ...
    startPath: function() {
                 ...
                 this.googlemaps-ap.closeInfoWindow();
                 this.dumpPaths();
	       },
...
// or just click the map to make a new marker.
    ...
    addMarkerToActivePath: function(marker) {
         ...
         if (this.paths_supported && this.active_path != undefined) {
             ...
             this.googlemaps-ap.addOverlay(marker.googlemaps-arker);
             this.dumpPaths();
         } else {
             this.singletons.addMarker(marker);
             this.googlemaps-ap.addOverlay(marker.googlemaps-arker);
...
// Could be removing from a path, or from the singletons
    ...
    removeActiveMarker: function() {
        if (this.active_marker.path) {
            ...
            this.zapgooglemaps-arker(this.active_marker);
            this.pruneOneMarkerPaths();
        } else {
            this.singletons.removeMarker(this.active_marker);
        }
        this.dumpPaths();
        this.googlemaps-ap.closeInfoWindow();
        this.zapgooglemaps-arker(this.active_marker.googlemaps-arker);
    },
    ...
    removeActiveMarkerAndJumpBack: function() {
       ...
       this.googlemaps-ap.returnToSavedPosition();
    },

    zapTempMarker: function() {
       if (this.temp_marker) {
           this.singletons.removeMarker(this.temp_marker);
           this.zapgooglemaps-arker(this.temp_marker.googlemaps-arker);
           this.temp_marker = undefined;
       }
   },
...
// order all the time, so I thought, what the hell.
    zapgooglemaps-arker: function(marker) {
		  GEvent.clearInstanceListeners(marker);
		  this.googlemaps-ap.removeOverlay(marker);
		},
...
/************** Search methods ***************/
    ...
    generateResultText: function(result) {
                         ...
                         text += '<a href="javascript:void(0)" onclick="emap.clipResult(\''+result.titleNoFormatting.replace(/'/g, '\\\'')+
                             '\', \''+this.generateResultTextCaption(result, '\\r\\n').replace(/'/g, '\\\'')+'\', '+result.lat+', '+result.lng+')">'+_['googlemaps-clip-result']+'</a>';
                         return text;
...
/************ Super-boring "set" methods **************/
    ...
    setControlSize: function(type) {
                      ...
                      this.googlemaps-ap.removeControl(this.current_control);
                      ...
                      if (this.current_control_type != "none") {
                        this.googlemaps-ap.addControl(this.current_control);
                      }
                    },
    ...
    setControl: function(which, whether) {
              ...
              if (whether == "yes") {
                  this.googlemaps-ap.addControl(this.controls[which]);
              } else {
                  this.googlemaps-ap.removeControl(this.controls[which]);
              }
    },
    ...
    setMapWidth: function(width) {
           ...
           if (width != parseInt(this.map_div.style.width)) {
             ...
             this.googlemaps-ap.checkResize();
           }
    },
    ...
    setType: function(type) {
                  ...
                  this.googlemaps-ap.setCenter(this.googlemaps-ap.getCenter(), this.googlemaps-ap.getZoom(), set_map_type);
             },
    ...
    setWorld: function(world, type) {
                  ...
                  this.world = world;
                  var map_types_orig = this.googlemaps-ap.getMapTypes().slice();
                  ...
                  for(i=0;i<map_types.length;i++) {
                      if (!map_types_orig_check[world+map_types[i].getName()]) {
                          this.googlemaps-ap.addMapType(map_types[i]);
                      }
                  }
                  ...
                  for(i=0;i<map_types_orig.length;i++) {
                      if (!map_types_check[old_world+map_types_orig[i].getName()]) {
                          this.googlemaps-ap.removeMapType(map_types_orig[i]);
                      }
                  }
                  this.setRadius(googlemaps-E_RADII[this.world]);
              },
...
// but only those which differ from defaults
       ...
       str += '&lt;googlemap';
       str += ' version="0.9"';
       str += ' lat="'+this.round(this.googlemaps-ap.getCenter().lat())+'"';
       str += ' lon="'+this.round(this.googlemaps-ap.getCenter().lng())+'"';
       var type = this.translateMapTypeToName(this.world, this.googlemaps-ap.getCurrentMapType());
       if (this.defaults.type != type)
	       str += ' type="'+type+'"';
       var world = this.world;
       if (this.defaults.world != world)
           str += ' world="'+world+'"';
       var zoom = this.googlemaps-ap.getZoom();
...
// the paths and spits out information about them
             ...
             if (this.paths[p] && this.selected_path == p) {
                 ...
                 if (p == this.active_path) {
                     str += '<div style="margin: 4px; clear: both;">'+
                         '<b>'+_['googlemaps-editing-path']+'</b>&nbsp;&nbsp;'+
                         '<a href="javascript:void(0)" onclick="emap.endPath()">'+_['googlemaps-save-path']+'</a>&nbsp;&nbsp;'+
                         '</div>';
                 } else {
    ...
    getLineLinksHtml: function(path, index) {
          ...
          str += '<a onclick="javascript:emap.activatePath('+index+')" href="javascript:void(0)">'+
              _['googlemaps-edit-path']+
              '</a>';
          str += ' - '+
              '<a onclick="javascript:emap.paths['+index+'].jump()" href="javascript:void(0)">'+
              _['googlemaps-show-path']+
              '</a>';

          if (path.poly) {
              str += ' - '+
                  '<a onclick="emap.paths['+index+'].removeFill()"'+
                  ' href="javascript:void(0)">'+_['googlemaps-remove-fill']+'</a>'+
                  '&nbsp;&nbsp;';
          } else {
              str += ' - '+
                  '<a onclick="emap.paths['+index+'].addFill()"'+
                  ' href="javascript:void(0)">'+_['googlemaps-add-fill']+'</a>';
          }
          str += '</div>';
          return str;
      },
    ...
    getLineOptionsHtml: function(path, index) {
         str = '';
         str += '<div style="float: left; clear: left;">'+_['googlemaps-line-color']+':</div>';
         str += '<div ';
         ...
         str += _['googlemaps-opacity']+': <select style="font-size: 10px;" onchange="emap.paths['+index+'].setLineOpacity(this.value, '+index+'); this.blur()">';
         for (i=0;i<=100;i+=10) {
             str += '<option ';
    ...
    getFillOptionsHtml: function(path, p) {
        ...
        str += '<div style="float: left;">'+_['googlemaps-fill-color']+':</div>';
        str += '<div ';
...
/********** Parser methods ************/
    ...
    listMaps: function() {
                ...
                for(var l=0; l < lines.length; l++) {
                  if (lines[l].match(/<googlemap[ >]/)) {
                    ...
                    if (attrs['name'] != undefined) {
                      ...
                      existing_maps[i] = _['googlemaps-map']+' #'+(i+1);
                    }
                    i++;
                  }
                }
                ...
                if (existing_maps[0]) {
                  map_selector_html = _['googlemaps-load-map-from-article']+' <select id="load_map_selector">';
                  ...
                  map_selector_html += '</select>&nbsp;&nbsp;'+
                      ...
                      _['googlemaps-load-map']+'</a>&nbsp;&nbsp;&nbsp;'+
                      '<a href="javascript:void(0)" onclick="javascript:emap.refreshMapList()">'+
                      _['googlemaps-refresh-list']+'</a>';
                  return map_selector_html;
                }
                return _['googlemaps-no-maps']+' <a href="javascript:void(0)" onclick="javascript:emap.refreshMapList()">'+_['googlemaps-refresh-list']+'</a>';
	      },
...
addXmlSource: function(url) {
                  ...
                  this.googlemaps-ap.addOverlay(geoxml);
              },
...
showXmlPoints: function(which) {
                   this.includes[which].overlay.gotoDefaultViewport(this.googlemaps-ap);
               },

removeXmlSource: function(which) {
                     this.kml_list.removeChild(this.kml_list.childNodes[which]);
                     this.googlemaps-ap.removeOverlay(this.includes[which].overlay);
